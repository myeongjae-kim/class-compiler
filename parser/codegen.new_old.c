    ...
    case OPERATOR:
      switch(code->whichval) {
        ...
        case AREFOP:
          ...
          sym_iter = code->symtype;
          while (sym_iter && sym_iter->kind != BASICTYPE) {
            sym_iter = sym_iter->datatype;
          }
          assert(sym_iter);
          if (sym_iter->basicdt == INTEGER) {
            if (code->symtype->size == 4) {
              asmstr(MOVL, reg-1,code->operands->link->intval, reg, "^.");
            } else {
              // size == 8
              asmstr(MOVQ, reg-1,code->operands->link->intval, reg, "^.");
            }
            
          } else {
            // double
            asmstr(MOVSD, XMM0,code->operands->link->intval, reg, "^.");
          }
